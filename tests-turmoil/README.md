...
**Build from this directory.** The workspace requires `--cfg tokio_unstable` (set in `.cargo/config.toml`, which cargo only reads when invoked from inside `tests-turmoil/`). Building from the repository root fails with a deliberate `compile_error!` instead of silently producing a non-deterministic binary.
