...
// Determinism depends on the vendored turmoil seeding tokio's runtime RNG
...
// directory — so fail loudly instead of silently losing reproducibility.
#[cfg(not(tokio_unstable))]
compile_error!(
    "tests-turmoil requires `--cfg tokio_unstable`; build from the tests-turmoil directory \
     so cargo picks up tests-turmoil/.cargo/config.toml"
);
