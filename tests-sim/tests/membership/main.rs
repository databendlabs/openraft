...
//! `select!` shuffle RNG at the start of every run. With `OPENRAFT_SIM_LOG=<file>`, rt-sim's
//! `sim-log` feature writes the full log of a run, identical for a given `OPENRAFT_RT_SIM_SEED`.
//!
