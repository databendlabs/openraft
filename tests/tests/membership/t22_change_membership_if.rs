...
type Clid = <LeaderId<u64, MemNodeId> as openraft::vote::RaftLeaderId>::Committed;
...
async fn committed_leader_id_guards_the_change() -> Result<()> {
    ...
    let established: Clid = {
        let metrics = leader.metrics().borrow_watched().clone();
        LeaderId::<u64, MemNodeId>::new_committed(metrics.current_term, metrics.current_leader.unwrap())
    };
    let other: Clid = LeaderId::<u64, MemNodeId>::new_committed(100, 2);
...
async fn last_log_id_guards_the_change() -> Result<()> {
    ...
    let metrics = leader.metrics().borrow_watched().clone();
    let leader_id = LeaderId::<u64, MemNodeId>::new_committed(metrics.current_term, metrics.current_leader.unwrap());
    let last_index = metrics.last_log_index.unwrap();
    let last_log_id = Some(LogId::new(leader_id, last_index));
    let earlier_log_id = Some(LogId::new(leader_id, last_index - 1));
