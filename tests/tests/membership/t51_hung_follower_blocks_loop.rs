...
use crate::fixtures::{RaftRouter, TypedRaftRouter};
...
async fn remove_hung_follower_must_not_block_raft_core_loop_1() -> Result<()> {
    ...
    TypeConfig::timeout(Duration::from_secs(10), membership_change).await.expect("membership change completed within the window")?;
...
async fn remove_hung_follower_must_not_block_raft_core_loop_2() -> Result<()> {
    ...
    TypeConfig::timeout(Duration::from_secs(10), write).await.expect("write completed within the window")?;
