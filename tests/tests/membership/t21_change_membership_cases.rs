...
async fn wait_for_leader_lease(router: &RaftRouter, leader_id: MemNodeId, mes: &str) -> anyhow::Result<()> {
    let metrics = router.get_metrics(&leader_id)?;
    if metrics.membership_config.voter_ids().eq([leader_id]) {
        return Ok(());
    }

    let leader = router.get_raft_handle(&leader_id)?;
    let old_acked = metrics.last_quorum_acked.map(|x| x.into_inner());

    leader.trigger().heartbeat().await?;
    router
        .wait(&leader_id, timeout())
        .metrics(
            |m| m.last_quorum_acked.map(|x| x.into_inner()) > old_acked,
            format!("leader establishes a fresh quorum lease, {}", mes),
        )
        .await?;
