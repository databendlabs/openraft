...
/// resolves with `OutcomeUnknown` and the node keeps applying.
#[tracing::instrument]
...
async fn write_then_superseded_by_snapshot_install() -> Result<()> {
    ...
    tracing::info!("--- the pending write must resolve with OutcomeUnknown, not panic the core");
    let write_res = rx.await?;
    let raft_err = write_res.unwrap_err();
    assert_eq!(
        raft_err,
        RaftError::APIError(ClientWriteError::OutcomeUnknown(ForwardToLeader {
            leader_id: Some(1),
            leader_node: Some(()),
