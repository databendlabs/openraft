...
async fn api_is_leader() -> Result<()> {
    let config = Arc::new(Config { enable_tick: false, ..Default::default() }.validate()?);
    ...
    for follower_id in [0, 1, 2].iter().filter(|&&id| id != leader_id) {
        ...
        assert!(!follower.is_leader(), "follower node {} should return false for is_leader()", follower_id);
    }
