...
async fn ensure_linearizable_process_from_followers() -> Result<()> {
    ...
    tracing::info!("--- block follower n1, write another log, n1 unable to apply last log, but n2 does");
    {
        ...
        let (read_log_id, leader_applied) = leader.get_read_log_id(ReadPolicy::ReadIndex).await?;
        assert_eq!(read_log_id.index(), Some(log_index), "read-log-id is the committed log");
        assert_eq!(leader_applied.index(), Some(log_index));
        ...
        let result = follower_n1.wait_for_apply(read_log_id, follower_n1_applied, Some(Duration::from_secs(1))).await?;
        assert_eq!(result, None, "follower n1 should wait timeout");

        let follower_n2 = router.get_raft_handle(&2).unwrap();
        let metrics = follower_n2.metrics().borrow().clone();
        let follower_n2_applied = metrics.last_applied;
        let result = follower_n2.wait_for_apply(read_log_id, follower_n2_applied, Some(Duration::from_secs(1))).await?;
        assert_eq!(
            result.index(),
            Some(log_index),
    ...
    tracing::info!("--- stop blocking, follower n1 will apply last log");
    {
        ...
        let metrics = follower_n1.metrics().borrow().clone();
        let follower_n1_applied = metrics.last_applied;
        let result = follower_n1.wait_for_apply(read_log_id, follower_n1_applied, None).await?;
        assert_eq!(
            result.index(),
            Some(log_index),
