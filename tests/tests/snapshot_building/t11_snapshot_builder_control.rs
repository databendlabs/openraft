...
/// Test snapshot builder control via `allow_build_snapshot` flag.
///
/// What does this test do?
///
/// - Build a stable single node cluster and apply some logs.
/// - Disable snapshot building via `allow_build_snapshot(false)`.
/// - Trigger snapshot building and verify it returns None without updating state.
/// - Re-enable snapshot building via `allow_build_snapshot(true)`.
/// - Trigger snapshot building again and verify it succeeds with state updated.
#[tracing::instrument]
#[test_harness::test(harness = ut_harness)]
async fn snapshot_builder_control() -> Result<()> {
    let config = Arc::new(
        Config {
