...
async fn heartbeat_does_not_trigger_false_reversion_panic() -> anyhow::Result<()> {
    let config = Arc::new(
        Config {
    ...
    tracing::info!(log_index, "--- trigger heartbeat on node-0 to check if panic occurs");
    {
        ...
        // Access RaftCore to make sure no panic happened.
