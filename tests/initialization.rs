...
fn initialization() {
    ...
    ctl.start_with_test(10, Box::new(|act, ctx| {
        ...
        let task = fut::wrap_future(act.network.send(ExecuteInRaftRouter(Box::new(move |act, _| {
            ...
            .and_then(|_, act, _| {
                let mut cmds: Vec<Box<dyn Future<Item=(), Error=InitWithConfigError>>> = vec![];
                cmds.push(Box::new(act.nodes.get(&0).expect("Expected node 0 to be registered with test controller.")
                    .send(InitWithConfig::new(vec![0, 1, 2])).map_err(|err| panic!(err)).and_then(|res| res)));
                ...
                fut::wrap_future(join_all(cmds)).map_err(|err, _, _| panic!(err))
            })
