...
//!
//! Provides [`LocalKey`] and the [`task_local!`] macro for declaring
...
impl<T: 'static> LocalKey<T> {
    ...
    where F: FnOnce() -> R {
        ...
        self.inner
            .try_with(|inner| inner.try_borrow_mut().map(|mut ref_mut| mem::swap(slot, &mut *ref_mut)))??;
...
impl<T: 'static, F: Future> Future for TaskLocalFuture<T, F> {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        let res = this.local.scope_inner(this.slot, || match future_opt.as_mut().as_pin_mut() {
            Some(fut) => {
                let res = fut.poll(cx);
                if res.is_ready() {
                    future_opt.set(None);
