    ...
    LS: RaftLogStorage<C>,
{
                        ...
                        ReplicationError::HigherVote(h) => {
                            let _ = self.tx_raft_core.send(Notify::Network {
                                response: Response::HigherVote {
                                    target: self.target,
                                    higher: h.higher,
                                    sender_vote: self.session_id.to_committed_vote(),
                                },
                            });
                            return Ok(());
                        }
    ...
    async fn send_log_entries(
        ...
        let payload = AppendEntriesRequest {
            vote: Vote::new_committed_from_leader_id(self.session_id.leader_id),
        ...
        let append_res = res.map_err(|_e| {
            let to = Timeout {
                ...
                id: self.session_id.leader_id.voted_for().unwrap(),
                target: self.target,
                timeout: the_timeout,
            };
            RPCError::Timeout(to)
        ...
        match append_resp {
            ...
            AppendEntriesResponse::HigherVote(vote) => {
                debug_assert!(
                    vote.leader_id() > &self.session_id.leader_id,
                    "higher vote({}) should be greater than leader's vote({})",
                    vote,
                    self.session_id.leader_id,
                );
    ...
    async fn stream_snapshot(
        ...
        let jh = AsyncRuntimeOf::<C>::spawn(Self::send_snapshot(
            ...
            self.session_id.to_committed_vote(),
            snapshot,
            option,
            rx_cancel,
    ...
    fn handle_snapshot_callback(
        ...
        let sender_vote = self.session_id.to_committed_vote();
