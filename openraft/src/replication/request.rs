...
where C: RaftTypeConfig
{
    ...
    Data {
        replication_id: Option<InflightId>,
        data: Data<C>,
    },
}
...
where C: RaftTypeConfig
{
    pub(crate) fn logs(log_id_range: LogIdRange<C>, replication_id: Option<InflightId>) -> Self {
        Self::Data {
            replication_id,
...
impl<C: RaftTypeConfig> fmt::Display for Replicate<C> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Data { replication_id, data } => {
                write!(f, "Data({}), replication_id:{}", data, replication_id.display())
            }
        }
    }
}
