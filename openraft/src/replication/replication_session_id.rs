...
/// For example, in a cluster of 3 nodes where node-1 is the leader, it maintains replication streams
/// to nodes {2,3}.
///
/// A replication session is uniquely identified by the leader's vote and the membership configuration.
/// When either changes, a new replication session is created.
///
