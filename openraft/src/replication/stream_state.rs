...
where
    ...
    LS: RaftLogStorage<C>,
{
    pub(crate) task_context: ReplicationContext<C>,
    ...
    LS: RaftLogStorage<C>,
{
    ...
    pub(crate) async fn next_request(&mut self) -> Option<AppendEntriesRequest<C>> {
        ...
        let (entries, sending_range) = match res {
            Ok(x) => x,
            Err(sto_err) => {
                tracing::error!("{} replication to target={}", sto_err, self.task_context.target);

                self.task_context.tx_notify.send(Notification::StorageError { error: sto_err }).await.ok();
                return None;
            }
        };
        ...
        let payload = AppendEntriesRequest {
            vote: self.task_context.session_id.vote(),
            prev_log_id: sending_range.prev.clone(),
            leader_commit: self.leader_committed.clone(),
            entries,
        };

        // TODO: add backoff if need to subscribe changes from RaftCore. now it does not need backoff
    ...
    async fn backoff_if_enabled(&mut self) {
        ...
        let cancel = self.task_context.cancel_rx.changed();
