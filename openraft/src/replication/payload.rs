...
where C: RaftTypeConfig
{
    ...
    ///
    /// A non-empty probe remains only when storage returned no entry. An open-ended stream
    /// advances to the last sent log id.
    #[must_use]
    pub(crate) fn update_sent(self, sent: Option<LogIdOf<C>>) -> Option<Self> {
        match self {
            Payload::Probe { log_id_range } => {
                if log_id_range.is_empty() {
                    return None;
                }

                if sent > log_id_range.prev {
                    return None;
                }
...
mod tests {
    ...
    #[test]
    fn probe_request_requires_an_entry() {
