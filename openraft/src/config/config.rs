...
pub struct Config {
    ...
    /// Duration in milliseconds of each heartbeat phase after a Leader loses quorum activity.
    ///
    /// `None` preserves the existing behavior. With `Some(R)`, heartbeats continue while the last
    /// quorum acknowledgement is newer than the Leader lease. After that lease expires, intervals
    /// of length `R` alternate between suppressing and allowing heartbeat attempts, starting with
    /// suppression. A new quorum acknowledgement restarts normal heartbeats.
    ///
    /// Before the first quorum acknowledgement, the current Vote's last-modified time is used.
    /// A single-voter Leader always sends heartbeats. Log replication and snapshots are unchanged;
    /// their responses may update the quorum-acknowledged time.
    ///
    /// `R` must be at least `leader_lease + election_timeout_max`. This is a nominal liveness
    /// floor, not a Raft safety requirement: every suppression interval leaves time for a
    /// previous follower lease to expire, then for one maximum election timeout. The Leader lease
    /// currently equals [`election_timeout_max`](Self::election_timeout_max). Add scheduling
    /// and network margin, covering every voter if nodes use different timing configurations. A
    /// delayed replication RPC may renew a follower lease after suppression begins, so the bound
    /// is not an unconditional liveness guarantee.
    ///
    /// This policy only gates heartbeat attempts. It does not schedule additional attempts, so
    /// automatic recovery attempts require periodic heartbeat to be enabled. ReadIndex and
    /// externally triggered heartbeats use the same gate. See
