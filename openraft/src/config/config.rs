...
pub struct Config {
    ...
    ///
    /// `None` disables quorum-loss inactivity and preserves the existing behavior. `Some(R)`
    /// enables it with recovery-probe interval `R`.
    ///
    /// An enabled Leader starts active and gathers voter responses for one Leader lease. When a
    /// due check finds no fresh voter quorum, it becomes inactive immediately: it keeps the same
    /// committed Vote and Leader identity, but pauses ordinary heartbeat, AppendEntries,
    /// snapshot, and ReadIndex-triggered heartbeat traffic. Already admitted RPCs may finish. A
    /// fresh quorum resumes traffic; a HigherVote follows the usual Leader transition. There is no
    /// additional grace period. This state is not persisted and a new Leader session starts active.
    ///
    /// `R` must be at least `leader_lease + election_timeout_max`. This is a nominal liveness
    /// floor, not a Raft safety requirement: it leaves time for a previous follower lease to
    /// expire, then for one maximum election timeout before probing again. The Leader lease
    ...
    /// delayed admitted RPC may renew a follower lease after inactivity begins, so the bound is
    /// not an unconditional liveness guarantee.
    ///
    ...
    /// Tick drives checks and probes. Disabling tick preserves activity state and deadlines;
    /// response-driven recovery and HigherVote handling continue. See
