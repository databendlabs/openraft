...
Set [`Config::quorum_loss_grace`][] and
[`Config::quorum_loss_probe_interval`][] to opt into pausing ordinary Leader
traffic when quorum stays unavailable. Both values are milliseconds and
default to `None`.

The current Leader session has three internal activity states:

- `Active`: normal traffic continues. A new Leader observes one full
  `leader_lease` window before its first quorum-loss check.
- `AwaitingQuorum`: the first failed check starts the configured grace period.
  Normal traffic continues. Any fresh quorum evidence immediately cancels the
  wait; quorum is checked again at the deadline before entering `Inactive`.
...
The configuration combinations are:

| Grace | Probe interval | Behavior |
|---|---|---|
| `None` | `None` | Disabled; existing behavior |
| `None` | `Some(R)` | Disabled; R is ignored and not validated |
| `Some(G)` | `None` | Invalid |
| `Some(G)` | `Some(R)` | Enabled after validation |

`G = 0` is valid. `R` must be strictly greater than
`leader_lease + election_timeout_max`: let the previous follower lease expire,
then leave one maximum election timeout before probing again. The Leader lease
currently equals `Config::election_timeout_max`. Add network and scheduling
margin and cover all voters' timing configurations. This conservative local
bound is not an unconditional liveness
guarantee: a delayed admitted RPC may renew a follower lease after the pause.
...
queued ticks and commands are not recalled. Responses and membership changes
may still restore quorum, and HigherVote processing continues. The next tick
after re-enabling evaluates the original deadlines without restarting them.

Leaving grace unset creates no activity controller, permit channel, or extra
quorum-loss checks. Existing evidence maintenance and RPC behavior are
unchanged. This default favors stable leadership during intermittent failures,
but old-Leader traffic can keep a bridge voter leased and prevent election in
the topology of [issue #2080](https://github.com/databendlabs/openraft/issues/2080).
Enabling inactivity gives the connected quorum an election opportunity at the
cost of possible pause/resume churn during unstable networking.

[`Config::quorum_loss_grace`]: crate::Config::quorum_loss_grace
[`Config::quorum_loss_probe_interval`]: crate::Config::quorum_loss_probe_interval
...
|----------|--------------------------|------------------|--------------------------|
| Lease expires | Become follower | Reject new proposals | Reject new proposals; await grace |
| Heartbeats and replication | Stop | Continue | Pause after grace; scheduled probes only |
