...
/// transfer, not the snapshot, and is gone; signatures live only in errors, which are never
/// stored, so unlike [`SnapshotMeta`](crate::storage::SnapshotMeta) the serialized form keeps
/// no reserved slot for it.
...
mod tests {
    ...
    fn test_snapshot_signature_serde() {
        ...
        assert_eq!(
            s,
            r#"{"last_log_id":{"leader_id":{"term":1,"node_id":2},"index":3},"last_membership_log_id":{"leader_id":{"term":4,"node_id":5},"index":6}}"#
        );
