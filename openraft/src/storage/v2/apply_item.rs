...
/// }
/// ```
pub struct ApplyItem<C: RaftTypeConfig> {
    pub(crate) entry: C::Entry,
    pub(crate) responder: Option<CoreResponder<C>>,
}

impl<C: RaftTypeConfig> ApplyItem<C> {
    ...
    pub fn into_parts(self) -> (C::Entry, ApplyResponder<C>) {
...
impl<C: RaftTypeConfig> std::fmt::Display for ApplyItem<C> {
