...
fn test_is_log_range_inflight() -> anyhow::Result<()> {
    ...
    pe.inflight = Inflight::snapshot(ReplicationId::new(0));
...
fn test_update_matching() -> anyhow::Result<()> {
    ...
    // Update matching and inflight
    {
        ...
        pe.new_updater(&engine_config).update_matching(Some(log_id(6)), Some(ReplicationId::new(0)));
        assert_eq!(inflight_logs(6, 10), pe.inflight);
        assert_eq!(Some(&log_id(6)), pe.matching());
        assert_eq!(20, pe.searching_end);

        pe.new_updater(&engine_config).update_matching(Some(log_id(10)), Some(ReplicationId::new(0)));
    ...
    // `searching_end` should be updated
    {
        ...
        pe.new_updater(&engine_config).update_matching(Some(log_id(20)), Some(ReplicationId::new(0)));
...
fn test_update_conflicting() -> anyhow::Result<()> {
    ...
    let engine_config = EngineConfig::new_default(1);
    pe.new_updater(&engine_config).update_conflicting(5, true, Some(ReplicationId::new(0)));
...
fn test_next_send() -> anyhow::Result<()> {
...

    {
        ...
        let res = pe.next_send(&mut new_raft_state(6, 10, 20), 100);
        assert_eq!(Ok(&Inflight::snapshot(ReplicationId::new(1))), res);
    }
    {
        //    matching,end
        //    4 6
