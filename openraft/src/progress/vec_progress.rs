    ...
    QS: QuorumSet<Id = Entry::Id>,
{
    ...
    /// Return a mutable entry without maintaining the progress ordering.
    ///
    /// Mutating the progress value through this method can leave the internal
    /// ordering and quorum-accepted value stale. Normal progress updates must
    /// use `update()`, `update_with()`, or `update_entry_with()` instead.
    pub(crate) fn get_mut_without_reorder(&mut self, id: &Entry::Id) -> Option<&mut Entry> {
        let index = self.index(id)?;
