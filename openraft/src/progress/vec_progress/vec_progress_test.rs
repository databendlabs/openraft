...
const KNOWN_ID_COUNT: u64 = 8;

fn known_ids() -> Vec<u64> {
    (0..KNOWN_ID_COUNT).collect()
}

fn draw_id(tc: &TestCase) -> u64 {
    tc.draw(generators::integers::<u64>().max_value(KNOWN_ID_COUNT))
}

/// A joint quorum set of one or two non-empty configs, as `Membership` builds.
fn draw_quorum_set(tc: &TestCase) -> Vec<BTreeSet<u64>> {
    let count = tc.draw(generators::integers::<usize>().min_value(1).max_value(2));
    (0..count)
        .map(|_| {
            let ids = tc.draw(generators::subsequences(known_ids()).min_size(1));
...
struct ProgressModel {
    progress: VecProgress<(u64, u64), Vec<BTreeSet<u64>>>,

    /// The expected quorum-accepted value.
    ///
    /// A value accepted by a quorum is never withdrawn, so this is the running maximum of the
    /// instantaneous model value rather than the model value itself. `upgrade_quorum_set` builds a
    /// new tracker that recomputes from scratch, which resets the running maximum.
    accepted: u64,
}
...
impl ProgressModel {
    fn check(&self, context: &str) {
        assert_eq!(
            self.accepted,
            *self.progress.quorum_accepted(),
            "{}: entries: {:?}",
            context,
            self.progress.entries
        );
        assert_voter_prefix_is_sorted(&self.progress, context);
    }

    fn advance_accepted(&mut self) {
        let model = model_quorum_accepted(&self.progress.quorum_set, &self.progress.entries);
        self.accepted = self.accepted.max(model);
    }
}
...
impl ProgressModel {
    ...
    fn update(&mut self, tc: TestCase) {
        let id = draw_id(&tc);
        let current = self.progress.try_get(&id).map(|entry| entry.1);
        let value = current.unwrap_or_default() + tc.draw(generators::integers::<u64>().max_value(7));

        let got = copy_result(self.progress.update(&id, value));

        self.advance_accepted();
        let want = if current.is_some() {
            Ok(self.accepted)
        } else {
            Err(self.accepted)
        };
        assert_eq!(want, got, "update({id}, {value}): entries: {:?}", self.progress.entries);
        self.check(&format!("update({id}, {value})"));
    }

    /// `increase_to` accepts a value below the current one and clamps itself.
    #[rule]
    fn increase_to(&mut self, tc: TestCase) {
        let id = draw_id(&tc);
        let value = tc.draw(generators::integers::<u64>().max_value(40));
        let current = self.progress.try_get(&id).map(|entry| entry.1);

        self.progress.increase_to(&id, value).ok();

        assert_eq!(
            current.map(|current| current.max(value)),
            self.progress.try_get(&id).map(|entry| entry.1),
            "increase_to({id}, {value})"
        );
        self.advance_accepted();
        self.check(&format!("increase_to({id}, {value})"));
    }

    /// `reset_entry_with` lowers a progress value, e.g. when a follower's log reverts. The
    /// quorum-accepted value must not follow it down.
    #[rule]
    fn reset_entry(&mut self, tc: TestCase) {
        let id = draw_id(&tc);
        let current = self.progress.try_get(&id).map(|entry| entry.1).unwrap_or_default();
        let value = tc.draw(generators::integers::<u64>().max_value(current));

        let got = self.progress.reset_entry_with(&id, |entry| entry.1 = value).copied();

        assert_eq!(
            self.progress.try_get(&id).map(|_| (id, value)),
            got,
            "reset_entry({id}, {value})"
        );
        self.check(&format!("reset_entry({id}, {value})"));
    }

    /// A membership change swaps the quorum set. The new tracker inherits every known node's
    /// progress and recomputes the quorum-accepted value from it.
    #[rule]
    fn upgrade_quorum_set(&mut self, tc: TestCase) {
        let quorum_set = draw_quorum_set(&tc);
        let learner_ids = learner_ids_for(&quorum_set, known_ids());

        let empty = VecProgress::new(vec![], [], |id| (id, 0));
        let old = std::mem::replace(&mut self.progress, empty);
        self.progress = old.upgrade_quorum_set(quorum_set, learner_ids, |id| (id, 0));

        self.accepted = model_quorum_accepted(&self.progress.quorum_set, &self.progress.entries);
        self.check("upgrade_quorum_set");
    }

    /// `is_voter` reports whether a tracked id can grant a value, and `None` for an untracked id.
    #[rule]
    fn is_voter(&self, tc: TestCase) {
        let id = draw_id(&tc);

        let want = if self.progress.quorum_set().ids().any(|voter| voter == id) {
            Some(true)
        } else {
            self.progress.try_get(&id).map(|_| false)
        };

        assert_eq!(want, self.progress.is_voter(&id));
    }

    #[invariant]
    fn quorum_accepted_matches_the_model(&self, _: TestCase) {
        self.check("invariant");
    }
}

/// Model test for the commit-index tracker: over generated joint quorum configurations, any
/// sequence of updates, resets and quorum-set upgrades keeps `quorum_accepted` equal to the
/// straightforward reference model, and keeps the descending voter prefix the optimized update
/// path relies on.
#[hegel::test]
fn vec_progress_matches_the_reference_model(tc: TestCase) {
    let quorum_set = draw_quorum_set(&tc);
    let learner_ids = learner_ids_for(&quorum_set, known_ids());
    let progress = VecProgress::<(u64, u64), _>::new(quorum_set, learner_ids, |id| (id, 0));

    hegel::stateful::run(ProgressModel { progress, accepted: 0 }, tc);
}
