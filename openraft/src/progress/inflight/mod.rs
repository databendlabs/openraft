...
where C: RaftTypeConfig
{
    ...
    ///
    /// Completes on the first acknowledgement beyond `prev`. Used for matching-point probes
    /// and the leader's local progress update, which acknowledges the entire range immediately.
    /// See [`LogIdRange::probe_completed_by()`] for the completion and retry rules.
    Logs {
