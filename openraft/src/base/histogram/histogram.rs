...
impl Histogram {
    ...
    ///
    /// Calculated as: bucket_index(u64::MAX) + 1 = 251 + 1 = 252
    ...
    fn percentile_with_total(&self, p: f64, total: u64) -> Option<u64> {
        ...
        for (bucket_index, &count) in self.buckets.iter().enumerate() {
            ...
            if cumulative >= target {
                return Some(self.bucket_min_values[bucket_index]);
            }
        }
    ...
    pub(crate) fn percentile_stats(&self) -> Option<PercentileStats> {
        let total = self.total();
        if total == 0 {
            return None;
        }
...
mod tests {
    ...
    fn test_percentile_empty() {
        ...
        assert_eq!(hist.percentile_stats(), None);
    }
    ...
    fn test_percentile_single_value() {
        ...
        assert_eq!(hist.percentile(0.99), Some(10));
        assert_eq!(hist.percentile(1.0), Some(10));
    }
    ...
    fn test_percentile_stats() {
        ...
        let stats = hist.percentile_stats().unwrap();
    ...
    fn test_percentile_large_values() {
        ...
        let p50 = hist.percentile(0.5).unwrap();
        assert!(p50 >= 96 && p50 <= 100, "P50 = {}", p50);

        // P80 should be the 4th value (1000), but bucket returns min value
        let p80 = hist.percentile(0.8).unwrap();
        assert!(p80 >= 896 && p80 <= 1000, "P80 = {}", p80);
    }
}
