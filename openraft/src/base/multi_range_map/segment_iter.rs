...
pub struct SegmentIter<'a, K, V, const N: usize>
where
    K: RangeMapKey,
    V: RangeMapValue,
{
    /// K-way merged, deduplicated boundary positions.
    merged_boundaries: Box<dyn Iterator<Item = K> + 'a>,

    /// Per-range-map peekable iterators yielding `(right_boundary, value)` pairs.
    right_value_iters: Vec<Peekable<vec_deque::Iter<'a, (K, V)>>>,

    /// Left boundary of the next segment to emit.
    left: K,
}

impl<'a, K, V, const N: usize> SegmentIter<'a, K, V, N>
where
    K: RangeMapKey + 'a,
    V: RangeMapValue,
{
    pub(crate) fn new(begin: K, range_maps: &'a [RangeMap<K, V>]) -> Self {
        ...
        let merged_boundaries: Box<dyn Iterator<Item = K> + 'a> = Box::new(
            range_maps
                .iter()
...
impl<K, V, const N: usize> Iterator for SegmentIter<'_, K, V, N>
where
    K: RangeMapKey,
    V: RangeMapValue,
{
    type Item = RangeValues<K, V, N>;
