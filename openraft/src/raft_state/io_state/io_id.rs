...
where C: RaftTypeConfig
{
    pub(crate) fn new(vote: Vote<C::NodeId>) -> Self {
        if vote.is_committed() {
            Self::new_append_log(vote.into_committed(), None)
        } else {
            Self::new_vote(vote.into_non_committed())
        }
    }

    pub(crate) fn new_vote(vote: NonCommittedVote<C>) -> Self {
        Self::Vote(vote)
    }

    pub(crate) fn new_append_log(committed_vote: CommittedVote<C>, last_log_id: Option<LogId<C::NodeId>>) -> Self {
    ...
    pub(crate) fn subject(&self) -> ErrorSubject<C> {
        match self {
            ...
            // Safe unwrap: new_append_log() won't create a None log id.
            Self::Log(log_io_id) => ErrorSubject::Log(log_io_id.log_id.unwrap()),
        }
    }
    ...
    pub(crate) fn verb(&self) -> ErrorVerb {
        match self {
            Self::Vote(_) => ErrorVerb::Write,
            Self::Log(_) => ErrorVerb::Write,
        }
    }
}
