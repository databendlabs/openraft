...
where C: RaftTypeConfig
{
    ...
    /// - **Update safety**: Forces callers to provide the leader's vote, preventing accidental updates
    ...
    ///    committed log id (vote-first protocol). Upon receiving the leader's committed log id,
    ///    the follower updates this value, then computes its local committed using
...
where C: RaftTypeConfig
{
    ...
    /// The safety check still exists in the code but always passes, serving as a defensive assertion.
    ///
    /// **For detailed explanation** with timeline examples, see:
    /// [Cluster-Committed vs Local Committed](crate::docs::protocol::commit)
    ...
    pub(crate) fn do_calculate_local_committed(&mut self) -> Option<LogId<C>> {
        ...
        // There may be enqueued IO that might truncate a committed log entry(and then may re-append it).
