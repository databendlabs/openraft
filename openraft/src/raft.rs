...
impl<D: AppData, R: AppDataResponse, N: RaftNetwork<D>, S: RaftStorage<D, R>> Raft<D, R, N, S> {
    ...
    /// If the original membership is {"members":{1,2,3}, "learners":{}},
    /// and call `change_membership` to {3,4,5}, then:
    /// - If `turn_to_learner` is true, then the new membership is {"members":{3,4,5}, "learners":{1,2}}.
