...
where C: RaftTypeConfig
{
    ...
    /// when purging logs.
    ///
    /// It returns an error only when RaftCore has [`Fatal`] error, e.g., shut down or having storage
    /// error.
    ///
    ...
    ///
    /// This method returns a [`Fatal`] error if it failed to send the request to RaftCore, e.g., when
