...
where C: RaftTypeConfig
{
    ...
    /// function, of type `V`, is returned wrapped in `Result<V, Fatal<C>>`. `Fatal` error will
    /// be returned if failed to receive a reply from `RaftCore`.
    ///
    /// A `Fatal` error is returned if:
    /// - Raft core task is stopped normally.
    /// - Raft core task is panicked due to programming error.
    ...
    /// }).await?;
    /// ```
    pub async fn with_state_machine<F, SM, V>(&self, func: F) -> Result<V, Fatal<C>>
    where
        SM: RaftStateMachine<C>,
        ...
        V: OptionalSend + 'static,
    {
        ...
        let got = self.inner.recv_msg(rx).await?;
        Ok(got)
    }
        ...
        F: FnOnce(&mut SM) -> BoxFuture<()> + OptionalSend + 'static,
    {
        ...
        let sm_cmd = sm::Command::Func { func };
