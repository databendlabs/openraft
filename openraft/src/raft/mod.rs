...
where C: RaftTypeConfig
{
    ...
    /// `get_read_log_id` from leader, then call [`wait_for_apply()`](Raft::wait_for_apply) to wait
    /// for the state machine to apply to the log id you specified.
    ///
    ...
    /// `wait_for_apply` wait for the state machine to apply entries up to
    /// `read_log_id`.
    ///
    /// If the current node's applied ID has reached or exceeded the read_log_id,
    /// there is no need to call this method. User can check it before call this method.
    ///
