...
where C: RaftTypeConfig
{
    ...
    /// submit transfer Leader command and the Leader will broadcast to every node in
    /// the cluster via [`RaftNetworkV2::transfer_leader`]. The application should then call this
    /// method.
    ///
    /// [`RaftNetworkV2::transfer_leader`]: crate::network::v2::RaftNetworkV2::transfer_leader
    #[since(version = "0.10.0")]
    pub async fn transfer_leader(&self, req: TransferLeaderRequest<C>) -> Result<(), Fatal<C>> {
