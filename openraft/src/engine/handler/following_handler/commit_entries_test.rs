...
fn test_following_handler_commit_entries_ge_accepted() -> anyhow::Result<()> {
    ...
    eng.state.io_state.io_progress.accept(IOId::new_append_log(committed_vote, Some(log_id(1, 1, 2))));
...
fn test_following_handler_commit_entries_le_accepted() -> anyhow::Result<()> {
    let mut eng = eng();
    let committed_vote = eng.state.vote_ref().into_committed();
    eng.state.io_state.io_progress.accept(IOId::new_append_log(committed_vote, Some(log_id(3, 1, 4))));
