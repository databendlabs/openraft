    ...
    SM: RaftStateMachine<C>,
{
    ...
    pub(crate) fn rebuild_progresses(&mut self) {
        ...
        let voter_quorum_changed = self.config.quorum_loss_grace.is_some()
            && self.leader.clock_progress.quorum_set().get_joint_config() != em.membership().get_joint_config();
        ...
        if voter_quorum_changed {
            ...
            if self.leader.is_inactive() {
                self.try_recover_activity(now);
            } else {
                self.leader.observe_quorum(now, leader_lease);
            }
        }
    }
    ...
    pub(crate) fn try_update_leader_clock(
        ...
        if self.config.quorum_loss_grace.is_some() {
            self.try_recover_activity(C::now());
        }
    ...
    pub(crate) fn try_recover_activity(&mut self, now: InstantOf<C>) {
        if self.leader.try_recover_activity(now, self.config.timer_config.leader_lease) {
