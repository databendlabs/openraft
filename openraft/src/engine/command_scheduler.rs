...
where C: RaftTypeConfig
{
    ...
    /// - The total entry count reaches `config.max_in_batch_entries`
    ///
    ...
    pub(crate) fn merge_front_append_entries(&mut self) {
        let max_entries = self.config.max_in_batch_entries();
...
mod tests {
    ...
    fn config_with_max_batch(max_in_batch_entries: u64) -> Config {
        Config {
            max_in_batch_entries: Some(max_in_batch_entries),
            ..Default::default()
        }
    }
    ...
    fn test_merge_stops_at_max_entries() {
        ...
        let config = config_with_max_batch(3);
        let mut output: EngineOutput<C> = EngineOutput::new(8);
        output.push_command(Command::AppendEntries {
            committed_vote: committed_vote(1, 0),
