    ...
    SM: RaftStateMachine<C>,
{
    ...
    pub(crate) fn handle_leader_activity(&mut self, now: InstantOf<C>) {
        let Some(grace) = self.config.quorum_loss_grace else {
            return;
        };
        ...
        let interval = self.config.quorum_loss_probe_interval.expect("enabled policy requires probe interval");

        if leader.is_inactive() {
            if leader.try_recover_activity(now, leader_lease) {
        ...
        } else if leader.check_activity(now, leader_lease, grace, interval) {
            self.output.prepend_command(Command::SetLeaderActivity {
                leader_vote: leader.committed_vote.clone(),
                active: false,
            });
    ...
    /// Re-anchor R after Core closes admission or actually submits a probe.
    pub(crate) fn arm_quorum_probe(&mut self, now: InstantOf<C>) {
        if let Some(interval) = self.config.quorum_loss_probe_interval
            && let Some(leader) = self.leader.as_mut()
        {
            leader.arm_quorum_probe(now, interval);
        }
    }
