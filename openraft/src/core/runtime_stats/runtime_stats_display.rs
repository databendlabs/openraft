...
pub struct RuntimeStatsDisplay {
    ...
    /// Pre-rendered `LogStages::segments()` output (type-erased from `LogStages<I>`).
    pub(crate) log_stages_display: String,
}
...
impl RuntimeStatsDisplay {
    ...
    fn fmt_multiline(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if !self.log_stages_display.is_empty() {
            writeln!(f, "  log_stage_segments:")?;
            for line in self.log_stages_display.lines() {
                writeln!(f, "    {}", line)?;
            }
        }
...
mod tests {
    ...
    fn test_multiline_with_log_stage_segments() {
        ...
        d.log_stages_display = format!("{}", log_stages);
