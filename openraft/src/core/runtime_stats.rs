...
impl RuntimeStatsDisplay {
    fn fmt_compact(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            ...
            self.apply_batch, self.append_batch, self.replicate_batch, self.raft_msg_batch, self.raft_msg_budget, self.notification_budget, self.raft_msg_usage_permille, self.notification_usage_permille
        )?;
