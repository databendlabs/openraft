...
where C: RaftTypeConfig<D = types_kv::Request, R = types_kv::Response, SnapshotData = Cursor<Vec<u8>>, Entry = Entry<C>>
{
    ...
    async fn build_snapshot(&mut self) -> Result<Snapshot<C>, io::Error> {
        ...
        let data = rkyv::to_bytes::<rkyv::rancor::Error>(&inner.state_machine)
...
where C: RaftTypeConfig<D = types_kv::Request, R = types_kv::Response, SnapshotData = Cursor<Vec<u8>>, Entry = Entry<C>>
{
    ...
    async fn install_snapshot(&mut self, meta: &SnapshotMeta<C>, snapshot: C::SnapshotData) -> Result<(), io::Error> {
        ...
        // Zero-copy validation and field access on snapshot bytes.
        let archived_state_machine =
            rkyv::access::<rkyv::Archived<StateMachineData>, rkyv::rancor::Error>(&new_snapshot.data)
                .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?;
        tracing::debug!(
            key_count = archived_state_machine.data.iter().count(),
            "validated snapshot bytes with zero-copy access"
        );

        let updated_state_machine = rkyv::deserialize::<StateMachineData, rkyv::rancor::Error>(archived_state_machine)
            .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?;

        let mut inner = self.0.lock().await;
        inner.last_applied_log = meta.last_log_id.clone();
        inner.last_membership = meta.last_membership.clone();
        inner.state_machine = updated_state_machine;
