...
async fn run_leader_distribution_test(node1_rafts: &[typ::Raft], node2_rafts: &[typ::Raft], node3_rafts: &[typ::Raft]) {
    ...
    // =========================================================================
    // Step 1: Initialize ALL groups on Node 1 with all 3 nodes as members
    // =========================================================================
    println!("=== Initializing all 3 groups on Node 1 ===\n");
    ...
    // Initialize all groups on Node 1
    node1_rafts[0].initialize(all_nodes.clone()).await.unwrap();
    println!("  ★ Group 'users'    initialized on Node 1 (all 3 nodes as voters)");

    node1_rafts[1].initialize(all_nodes.clone()).await.unwrap();
    println!("  ★ Group 'orders'   initialized on Node 1 (all 3 nodes as voters)");

    node1_rafts[2].initialize(all_nodes.clone()).await.unwrap();
    println!("  ★ Group 'products' initialized on Node 1 (all 3 nodes as voters)");

    tokio::time::sleep(Duration::from_millis(1000)).await;

    // Step 2: Use transfer_leader to distribute leaders to different nodes
    // =========================================================================
    println!("\n=== Using transfer_leader to distribute leaders ===\n");

    // Transfer 'orders' leader to Node 2
    println!("  → Transferring 'orders' leader from Node 1 to Node 2...");
    node1_rafts[1].trigger().transfer_leader(2).await.unwrap();
    tokio::time::sleep(Duration::from_millis(1000)).await;
    println!("  ★ Group 'orders' leader transferred to Node 2");

    // Transfer 'products' leader to Node 3
    println!("  → Transferring 'products' leader from Node 1 to Node 3...");
    node1_rafts[2].trigger().transfer_leader(3).await.unwrap();
    tokio::time::sleep(Duration::from_millis(1000)).await;
    println!("  ★ Group 'products' leader transferred to Node 3");
