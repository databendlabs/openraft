    ...
    async fn build_snapshot(&mut self) -> Result<Snapshot<TypeConfig>, StorageError<TypeConfig>> {
        ...
        let data = spawn_blocking(move || {
            ...
            serialize(&snapshot_data)
                .map_err(|e| StorageError::write_snapshot(Some(meta_clone.signature()), AnyError::new(&e)))
        })
        .await
        ...
        fs::write(&snapshot_path, &data).map_err(|e| StorageError::write_snapshot(Some(meta.signature()), &e))?;
...
impl RaftStateMachine<TypeConfig> for RocksStateMachine {
    ...
    async fn install_snapshot(
        ...
        // Write snapshot file for get_current_snapshot
        let snapshot_path = self.snapshot_dir.join(&meta.snapshot_id);
        fs::write(&snapshot_path, snapshot.get_ref())
    ...
    async fn get_current_snapshot(&mut self) -> Result<Option<Snapshot<TypeConfig>>, StorageError<TypeConfig>> {
        ...
        let entries = fs::read_dir(&self.snapshot_dir).map_err(|e| StorageError::read_snapshot(None, &e))?;

        let mut latest_snapshot: Option<(String, PathBuf)> = None;
            ...
            if path.is_file() {
                if let Some(filename) = path.file_name().and_then(|n| n.to_str()) {
                    ...
                    match &latest_snapshot {
                        None => latest_snapshot = Some((snapshot_id, path)),
                        Some((current_id, _)) => {
                            if snapshot_id > *current_id {
                                latest_snapshot = Some((snapshot_id, path));
                            }
                        }
                    }
                }
        ...
        let Some((snapshot_id, snapshot_path)) = latest_snapshot else {
            return Ok(None);
        };

        // Read snapshot data from file
        let data = fs::read(&snapshot_path).map_err(|e| StorageError::read_snapshot(None, &e))?;

        // Get metadata from state machine
        let (last_applied_log, last_membership) = self.get_meta()?;
