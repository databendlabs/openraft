...
impl ExampleClient {
    ...
    pub async fn read(&self, req: &String) -> Result<String, RPCError> {
        let x: Result<_, Infallible> = self.do_send_rpc_to_leader("read", Some(req)).await?;
        Ok(x.unwrap())
    }
    ...
    /// See [`RaftMetrics`].
    pub async fn metrics(&self) -> Result<RaftMetrics<TypeConfig>, RPCError> {
        let x: Result<RaftMetrics<TypeConfig>, Infallible> = self.do_send_rpc_to_leader("metrics", None::<&()>).await?;
        Ok(x.unwrap())
    }
