...
// Adding rkyv here will result in errors
...
pub struct StateMachineData {
    ...
    pub last_membership: Option<Membership>,
...
impl RaftSnapshotBuilder<TypeConfig> for Arc<StateMachineStore> {
    ...
    async fn build_snapshot(&mut self) -> Result<Snapshot, io::Error> {
...

        {
            ...
            // Here we are using rkyv to serialize
            data = rkyv::to_bytes::<rkyv::rancor::Error>(&*state_machine).unwrap();
        }
        ...
        // We use to_vec() instead of clone (which does mostly the same thing) as Rkyv uses AlignedVec
...
impl RaftStateMachine<TypeConfig> for Arc<StateMachineStore> {
    ...
    where Strm: Stream<Item = Result<EntryResponder<TypeConfig>, io::Error>> + Unpin + OptionalSend {
        ...
        while let Some((entry, responder)) = entries.try_next().await? {
            let log_id: LogId = <Entry as RaftEntry<TypeConfig>>::log_id(&entry).clone();
    ...
    async fn install_snapshot(&mut self, meta: &SnapshotMeta, snapshot: SnapshotData) -> Result<(), io::Error> {
        ...
        {
            // This operation copies (defeating the purpose of rkyv) but is required to set the state
            // In a actual deployment, we do other things with StateMachineData than just save it.
            let d: StateMachineData =
                rkyv::from_bytes::<StateMachineData, rkyv::rancor::Error>(&new_snapshot.data.as_ref())
