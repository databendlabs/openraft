...
impl RaftSnapshotBuilder<TypeConfig> for Arc<StateMachineStore> {
    ...
    async fn build_snapshot(&mut self) -> Result<Snapshot, StorageError> {
...

        {
            ...
            let last_membership_log_id = state_machine.last_membership_log_id.clone().map(|x| x.into());
            let membership = state_machine.last_membership.clone().unwrap().into();
...
impl RaftStateMachine<TypeConfig> for Arc<StateMachineStore> {
    ...
    async fn applied_state(&mut self) -> Result<(Option<LogId>, StoredMembership), StorageError> {
        ...
        let last_applied = sm.last_applied.clone().map(|x| x.into());

        let mem = StoredMembership::new(
            sm.last_membership_log_id.clone().map(|x| x.into()),
            sm.last_membership.clone().unwrap().into(),
        );
