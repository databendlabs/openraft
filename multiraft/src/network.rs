...
//! - [`GroupNetworkAdapter`] - Wraps `GroupedRpc` with (target, group_id), implements
//!   `RaftNetworkV2`
...
pub trait GroupedRpc<C, G>: Clone + OptionalSend + OptionalSync + 'static
...
///
/// This wraps a [`GroupedRpc`] implementation (e.g., your Router) and
    ...
    N: GroupedRpc<C, G>,
{
    async fn append_entries(
        ...
        self.router.send_append_entries(self.target.clone(), self.group_id.clone(), rpc, option).await
    }

    async fn vote(&mut self, rpc: VoteRequest<C>, option: RPCOption) -> Result<VoteResponse<C>, RPCError<C>> {
        self.router.send_vote(self.target.clone(), self.group_id.clone(), rpc, option).await
    }
    ...
    async fn transfer_leader(&mut self, req: TransferLeaderRequest<C>, option: RPCOption) -> Result<(), RPCError<C>> {
        self.router.send_transfer_leader(self.target.clone(), self.group_id.clone(), req, option).await
    }
