//! Extended Raft with `install_snapshot()` for receiving snapshot chunks.
//!
//! This module provides [`RaftV1`], a wrapper around [`openraft::Raft`] that adds
...
/// Extended Raft with `install_snapshot()` for chunk-based snapshot receiving.
///
...
/// ```
pub struct RaftV1<C: RaftTypeConfig> {
    inner: OpenRaft<C>,
}

impl<C: RaftTypeConfig> RaftV1<C> {
    /// Create a new `RaftV1` wrapper around an [`openraft::Raft`] instance.
    pub fn new(inner: OpenRaft<C>) -> Self {
        Self { inner }
    }
    ...
    /// 2. Receiving the chunk via `Chunked::receive_snapshot()`
        ...
        C::SnapshotData: tokio::io::AsyncRead + tokio::io::AsyncWrite + tokio::io::AsyncSeek + Unpin,
    {
        ...
        let snapshot = Chunked::<C>::receive_snapshot(&mut *streaming, &self.inner, req).await?;
...
impl<C: RaftTypeConfig> Clone for RaftV1<C> {
    fn clone(&self) -> Self {
        Self {
...
impl<C: RaftTypeConfig> std::ops::Deref for RaftV1<C> {
    type Target = OpenRaft<C>;
